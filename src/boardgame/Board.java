...
public class Board {
	...
	public Piece piece(int row, int column) {
		if (!positionExists(row, column)) {						// programação defensiva: verifica se a posição não existe. Em caso positivo, mostra a mensagem de erro
			throw new BoardException("Nao existe esta posição no tabuleiro");
		}
	...
	public Piece removePiece(Position position) {
		...
		if (piece(position) == null) {				// testa se a posição está vazia. Em caso positivo, não faz nada
			return null;
		}
		Piece aux = piece(position);
		aux.position = null;
		pieces[position.getRow()][position.getColumn()] = null;		// informa que, na linha e coluna especificada, não há peça (foi removida)
		return aux;	
	}
